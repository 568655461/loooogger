 ...
 * zhuge 2018-11-07 20:10
 * last modify time: 2018-12-11 17:42
 * last modify author: zhuge
 */
...
var mkdirSync = require('../util/mkdirsSync');
/**
 * 
 * 删除过期的错误ejs日志标志
...
function deleteTagsFile (logPath, filename) {
  // 遍历当前标志文件夹所有文件
  var checkFileIsExists = fs.existsSync(logPath);
  var currentFiles = []; // 所有文件
  if (checkFileIsExists) {
    currentFiles = fs.readdirSync(logPath);
  ...
  currentFiles.forEach(function (files, filesIndex) {
    if (files !== filename) {
      //删除文件
      fs.unlink(logPath + '/' + files, function (err) {
        if (err) {
...
function checkLogToSend (logs) {
    ...
    var logPath = config.getConfig().emailCachePath;
    console.log('=============日志错误文件缓存路径=============================');
    console.log(logPath);
    var filename = moment().format('YYYY-MM-DD') + '.txt';
    var cacheFile = logPath + filename;
    // 同步创建文件目录
    mkdirSync.mkdirsSync(logPath);
    ...
    var writeAndSend = function (err, data) {
      if (err) {
        ...
        // 发送邮件
        email.sendEmail(config.getConfig().project+'项目：ejs渲染错误，出现空白页面！！！',logs,null);
        //保存到文件
